...
import { ArrowLeft, BookOpen, FileSearch, ChevronDown, Loader2 } from 'lucide-react'
...
// ─── Question Card (same logic as student view – parent read-only) ─────
  ...
  return (
    <div
      className={cn(
        ...
        expanded ? 'border-[#2D5BFF]/40 shadow-md' : 'border-[#E5E7EB] shadow-sm',
      )}
    >
      <button
      ...
      >
        <div className="w-9 h-9 rounded-xl bg-[#E8EDFF] flex items-center justify-center shrink-0">
          <span className="text-sm font-bold text-[#2D5BFF]">{index}</span>
        </div>
        <div className="flex-1 min-w-0">
          ...
          {question.maxPoints != null && question.maxPoints > 0 && (
            <p className="text-[12px] text-[#2D5BFF] font-medium mt-0.5">
...
// ─── Page ─────────────────────────────────────────────────────────
export default function ParentExamDetailPage(props: {
  ...
  const backHref = studentId
    ? `/parent/student/${studentId}/exams`
    : '/parent/dashboard'

  const { data: questions, error, isLoading } = useSWR(
    studentId ? `parentExamQuestions-${studentId}-${params.id}` : null,
    () => api.getParentExamQuestions(parseInt(studentId!), parseInt(params.id))
  )

  if (!studentId) {
    return <div className="p-6 text-red-500">Öğrenci bilgisi eksik.</div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-[#111827]">Sınav İnceleme</h1>
          <p className="text-sm text-[#6B7280]">Sınav #{params.id} – Soru bazlı değerlendirme</p>
        </div>
      </div>

      {/* Summary bar */}
      <div className="bg-[#E8EDFF] rounded-2xl p-4">
        <div className="flex items-center gap-3 mb-3">
          <FileSearch size={18} className="text-[#2D5BFF]" />
          <span className="text-[14px] font-semibold text-[#2D5BFF]">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-xl p-3 text-center">
            <BookOpen size={18} className="text-[#2D5BFF] mx-auto mb-1" />
          ...
          {totalMax > 0 && (
            <div className="bg-white rounded-xl p-3 text-center">
              <p className="text-xl font-bold text-[#2D5BFF]">
      ...
      <div className="space-y-3">
        {questions.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-[#E5E7EB]">
            ...
            <p className="text-[#111827] font-medium">Soru Bulunamadı</p>
            <p className="text-[#6B7280] text-sm mt-1">
              Bu sınav için henüz soru işlenmemiş.
            </p>
          </div>
        ) : (
          questions.map((q, i) => (
